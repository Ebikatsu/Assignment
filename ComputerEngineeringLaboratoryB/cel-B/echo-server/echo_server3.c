...
*/
	...
	//戻り値 受信したバイト数。失敗したら-1
        ...
        //クライアントから送られてきた文字列recvStrを逆順に並べた文字列sendStrを作成する
        memset(sendStr, '\0', BUFFER_LEN*sizeof(char));
        for(i = 0; i < strlen(recvStr) - 1; i++)sendStr[i] = recvStr[strlen(recvStr) - i - 2];
        sendStr[strlen(recvStr) - 1] = '\n';
	...
	//戻り値 送信したバイト数。失敗したら-1
        /*** メッセージを送信する ***/
        numsnt = send(clientSocket, sendStr, strlen(sendStr) + 1, 0);
